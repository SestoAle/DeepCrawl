...
public class BoardManagerSystem : MonoBehaviour
{
    ...
    public bool saveInformation = false;
    public int maxEpisodeInSavingInformations = 200;
    [Space(10)]

    //ML informations
    [Header("---BT Informations---")]
    public bool behaviorTree;
    public bool MLvsBT;
    [Space(10)]

    //ML informations
    [Header("---Guided Learning Informations---")]
    [Tooltip("When isGuided is ON, the agent must arrive at one point " +
        "in the map. There is no opponent.")]
    ...
    void Start()
    {
        ...
        // Create Level object and all containers.
        Level = new GameObject("Level");
        OuterContainer = new GameObject("OuterContainer");
        OuterContainer.transform.parent = Level.transform;
        // Check the informations relative to what type of training is
        if (isGuided)
        {
            isTraning = true;
        }
        ...
        // If StartScene, create only one enemy
        ...
        if (sceneName == "StartScene")
        {
            GameManager.instance.gameUI.moveCameraAtPosition(activePlayer.transform.position.x, activePlayer.transform.position.z);
            currentRoomId = 0;
        }
    }
    ...
    void generateCharacters()
    {
        ...
        else
        {
            ...
            if (player)
                ...
                GameObject.Find("Enemy 2").tag = "Puppet";
            else if (MLvsBT)
                GameObject.Find("Enemy 2").tag = "TreeAgent";
            else if (doubleAgent)
                GameObject.Find("Enemy 2").tag = "DoubleBrainedAgent";
            else
                GameObject.Find("Enemy 2").tag = "Random";

            GameObject.Find("Enemy 1").tag = "BrainedAgent";

            if (behaviorTree && !MLvsBT)
            {
                GameObject.Find("Enemy 1").tag = "TreeAgent";
            }

            if (puppet)
            {
                GameObject.Find("Enemy 1").tag = "Puppet";
            }
            ...
            // IRL settings
            // If isGuided is ON, both the enemy and the agent will always be at the same position.
            if (isGuided)
            {
                //spawnAtPosition(10, 10, GameObject.Find("Enemy 2"));
                randomSpawn(GameObject.Find("Enemy 2"));
                //spawnAtPosition(19, 19, GameObject.Find("Enemy 1"));
            }
    ...
    public void createStartEnemy()
    {
        ...
        activePlayer = createCharacter(ArcherPrefab, x, y, 0, DIRECTION.North);

        entityManager.SetComponentData(activePlayer.GetComponent<Character>().Entity, new Position { x = x, y = y });
        entityManager.AddComponentData(activePlayer.GetComponent<Character>().Entity, new Player { });

        activePlayer.name = "StartAgent";
        activePlayer.tag = "BrainedAgent";
        activePlayer.GetComponent<BaseAgent>().GiveBrain(ArcherBrain);
        activePlayer.GetComponent<Character>().target = activePlayer;
    }
    ...
    public void randomSpawn(GameObject character)
    {
        ...
        Tile characterTile = getTileFromObject(character);
    ...
    // Move a character in a specific position of the map. It is used only for
    // training.
    public void spawnAtPosition(int x, int y, GameObject character)
    {
        Tile prevTile = getTileFromObject(character);
        if (prevTile.hasCharacter())
            prevTile.setCharacter(null);
        Tile t = getTile(x, y);
        character.transform.position = new Vector3(x, 0, y);
        t.setCharacter(character);
        entityManager.SetComponentData(character.GetComponent<Character>().Entity, new Position { x = x, y = y });
    ...
    public void resetTraining()
    {
        ...
        for (int i = 0; i < numAgents; i++)
        {
...
#if UNITY_EDITOR
            // Save Agent Informations
            if(saveInformation)
            {
                if (character.GetComponent<BaseAgent>() != null && character.GetComponent<BaseAgent>().getAgentInfos() != null)
                {
                    character.GetComponent<BaseAgent>().getAgentInfos()["episodes"] += 1;
                    character.GetComponent<BaseAgent>().getAgentInfos()["max_episodes"] = maxEpisodeInSavingInformations;
                    character.GetComponent<BaseAgent>().printAgentInfos();
                    if(character.GetComponent<BaseAgent>().getAgentInfos()["episodes"] >= maxEpisodeInSavingInformations)
                    {
            ...
            if(noAnim)
            {
                if (character.gameObject.GetComponent<Animator>().enabled)
                {
            ...
            if (character.tag == "BrainedAgent" || character.tag == "TreeAgent")
            {
                ...
                // TODO: change the way we assign MP
                stats = new Stats { hp = actualHp, atk = atk, def = def, des = des, maxHp = 20, mp = 10, maxMp = 10};
            ...
            else
            {
                ...
                int actualHp = Random.Range(hp, maxHp + 1);
                // Todo: change the way we assign MP
                stats = new Stats { hp = actualHp, atk = 3, def = 3, des = 3, maxHp = 20, mp = 10, maxMp = 10};
                // Create a random inventory
                createStartInventory(character.GetComponent<Inventory>(), true);
            ...
            if (isGuided)
            {
                ...
                {
                    randomSpawn(character);
                    //spawnAtPosition(10, 10, character);
                }
                else
                {
                    //randomSpawn(character);
                    spawnAtPosition(19, 19, character);
                }
            }
        }
    ...
    public List<Tile> findPath(Tile startTile, Tile endTile, bool withItem, List<Tile> avoidTiles)
    {
        ...
        while (allTileDist.Count > 0)
        {
            ...
            {
                if ((!neigh.canMove() && neigh != endTile) || (neigh.hasItem() && !withItem && neigh != endTile))
                    continue;
    ...
    {
        return findPath(startTile, endTile, false, null);
    }
    ...
    // From offset get Movement UserInput
    public UserInput offsetToMovementUserInput(Vector3 offset)
    {
        // Movement actions (0-7)
        if ((int)offset.x == 0 && (int)offset.z == 1)
        {
            return new UserInput { action = 0 };
        }
        if ((int)offset.x == 1 && (int)offset.z == 0)
        {
            return new UserInput { action = 1 };
        }
        if ((int)offset.x == 0 && (int)offset.z == -1)
        {
            return new UserInput { action = 2 };
        }
        if ((int)offset.x == -1 && (int)offset.z == 0)
        {
            return new UserInput { action = 3 };
        }
        if ((int)offset.x == 1 && (int)offset.z == 1)
        {
            return new UserInput { action = 4 };
        }
        if ((int)offset.x == 1 && (int)offset.z == -1)
        {
            return new UserInput { action = 5 };
        }
        if ((int)offset.x == -1 && (int)offset.z == -1)
        {
            return new UserInput { action = 6 };
        }
        if ((int)offset.x == -1 && (int)offset.z == 1)
        {
            return new UserInput { action = 7 };
        }
    ...
    public List<Tile> getRangeTiles(int range, Tile startTile, GameObject character)
    {
        ...
        foreach (DIRECTION dir in System.Enum.GetValues(typeof(DIRECTION)))
        {
            ...
            {
                newTile = getTile((int)(prevTile.x + offset.x), (int)(prevTile.y + offset.y));
