...
public class BaseAgent : Agent
{
    ...
    // Agent Information
    Dictionary<string, int> agentInfos;

    private void Start()
    {
        AgentRestart();
        agentInfos = new Dictionary<string, int>();
        agentInfos.Add("max_episodes", -1);
        agentInfos.Add("episodes", 0);
        agentInfos.Add("melee", 0);
        agentInfos.Add("range", 0);
        agentInfos.Add("buff", 0);
        agentInfos.Add("health", 0);
        agentInfos.Add("win", 0);

        for (int i = 0; i < 17; i ++)
        {
            agentInfos.Add(i.ToString(), 0);
        }
    }
    ...
    public override void AgentAction(float[] vectorAction, string textAction)
    {
        ...
        if ((BoardManagerSystem.instance.doubleAgent && gameObject.tag == "DoubleBrainedAgent") || brain.brainType == BrainType.Internal)
        {
        ...
        {
            if (BoardManagerSystem.instance.doubleAgent && gameObject.tag == "DoubleBrainedAgent")
            {
    ...
    public void giveDamageReward()
    {
        ...
        if (currentTargetHp <= 0)
        {
            ...
            Done();
            if(BoardManagerSystem.instance.isTraning && !BoardManagerSystem.instance.noAnim)
            {
                //BoardManagerSystem.instance.resetTraining();
            }
        }
    ...
    public Dictionary<string, int> getAgentInfos()
    {
        return agentInfos;
    }

    public void printAgentInfos()
    {

        string path = "Assets/Resources/agentInfo.txt";
        ...
        foreach (KeyValuePair<string, int> info in agentInfos)
        {
            writer.WriteLine(info.Key + ", " + info.Value.ToString());
        }
