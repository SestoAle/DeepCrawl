...
public class ActionSystem : ComponentSystem
{
    ...
    {
        startPos = new Vector2(startTile.x, startTile.y);
    ...
    protected override void OnUpdate()
    {
        ...
        for (var i = 0; i < data.Length; i++)
        {
            ...
            if (character.GetComponent<BaseAgent>() != null)
            {
                // Add a melee count to the agent
                BaseAgent agentComponent = character.GetComponent<BaseAgent>();
                Dictionary<string, int> agentInfos = agentComponent.getAgentInfos();

                agentInfos[userInput.action.ToString()] += 1;
            }
            ...
            switch (userInput.action)
            {
                ...
                case 8:
                    ...
                    if (!EntityManager.HasComponent(entity, typeof(Buff)))
                    {
                        ...
                        if (potion != null)
                        {
                            ...
                            else if (potionComponent is MagicPoint)
                            {
                                puc.AddComponent(entity, (MagicPoint)potionComponent); 
                            }
                            // destroy the potion
                ...
                // Magic Action
                // If it is a magic action, crate the magic component
                case 17:
                    puc.AddComponent(entity, new Magic{damage = 5, type = 0, mp = 3});
                    break;
                case 18:
                    puc.AddComponent(entity, new Magic{damage = 5, type = 1, mp = 3});
                    break;
            }
            ...
            if (newPos != oldPos)
            {
                ...
                else if (newTile != null && newTile.hasCharacter() && tile.isNeighbour(newTile))
                {
                    ...
                    if (!character.CompareTag(((GameObject)newTile.getCharacter()).tag))
                    {
                        if(BoardManagerSystem.instance.saveInformation)
                        {
                            if(character.GetComponent<BaseAgent>() != null)
                            {
                                // Add a melee count to the agent
            ...
            if (rangeTile != null && rangeTile.hasCharacter() && tile.parent == rangeTile.parent)
            {
                ...
                {
                    // If is not an ally
                    if (!character.CompareTag(((GameObject)rangeTile.getCharacter()).tag))
                    {
                        if (character.GetComponent<BaseAgent>() != null)
                        {
                            // Add a melee count to the agent
            ...
            if (!BoardManagerSystem.instance.noAnim && character.CompareTag("Player"))
            {
